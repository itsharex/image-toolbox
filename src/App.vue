...
const currentItem = computed(() => {
  for (const group of navGroups) {
    const found = group.items.find(i => i.id === activeTab.value);
...
<template>
  <div class="flex h-screen w-screen bg-background text-zinc-100 font-sans selection:bg-primary/30 selection:text-white overflow-hidden">
    ...
    <aside class="w-[260px] flex flex-col border-r border-white/5 bg-zinc-900/50 backdrop-blur-2xl z-20 relative">
      ...
      <!-- Navigation -->
      <nav class="flex-1 px-4 space-y-6 overflow-y-auto mt-2">
        <div v-for="group in navGroups" :key="group.title" class="space-y-1.5">
             <div class="px-3 mb-2">
                <span class="text-[10px] uppercase tracking-wider text-zinc-600 font-semibold">{{ group.title }}</span>
             </div>
